...
def test_valid_payload_returns_200():
    assert client.post("/api/auth", json=_make_payload()).status_code == 200


def test_valid_payload_returns_token():
    r = client.post("/api/auth", json=_make_payload())
...
def test_optional_fields_accepted():
    ...
    assert client.post("/api/auth", json=payload).status_code == 200
...
def test_wrong_hash_returns_401():
    ...
    payload["hash"] = "a" * 64
    assert client.post("/api/auth", json=payload).status_code == 401
...
def test_wrong_bot_token_returns_401(monkeypatch):
    ...
    assert client.post("/api/auth", json=_make_payload()).status_code == 401
...
def test_missing_id_returns_422():
    payload = _make_payload()
    del payload["id"]
    assert client.post("/api/auth", json=payload).status_code == 422
